...
directions. Normalizing flows learn the Boltzmann distribution for efficient
sampling~\cite{Albergo2019,Kanwar2020,Boyda2021}. Gauge-equivariant
convolutional neural networks enforce exact lattice symmetries in the network
architecture~\cite{Favoni2022,Luo2021}. Graph neural networks have been applied
to learn field-theoretic observables~\cite{Bachtis2021}, and relational
inductive biases have proven valuable in broader physics
contexts~\cite{Cranmer2020,Battaglia2018}.
...
types connected by typed edges. This mirrors the fiber bundle structure of
continuum field theory: spacetime nodes play the role of the discrete base
manifold $M$, field nodes correspond to sections of the associated fiber bundle,
and the ``inhabits'' edges encode the projection map $\pi: E \to M$ associating
each field value to its spacetime site. The three-stage message-passing protocol
...
as a single-channel image (4 layers, 32 channels).
...
\midrule
\textbf{HeteroGNN (ours)} & 150,337 & \textbf{0.99998} & \textbf{0.04\%} \\
Homogeneous GNN & 67,073 & 0.99985 & 0.12\% \\
Lattice CNN & 29,153 & 0.99855 & 0.39\% \\
MLP & 197,633 & 0.346 & 7.4\% \\
\bottomrule
\end{tabular}
\end{table}
...
destroys the field signal entirely ($r < 0.05$). This workaround is not physically motivated---it is an engineering patch
required because field values and geometry features share a single node
representation that is progressively smoothed during message passing. The bipartite architecture avoids this failure mode by
construction: field nodes maintain a separate representation throughout the
network, ensuring the field signal reaches the readout regardless of
message-passing depth. Note that the per-stage residual connections in the
...
$r > 0.998$, confirming that the $\phifour$ action is learnable as a local
functional. The HeteroGNN achieves the lowest relative error (0.04\%),
a 3$\times$ improvement over the homogeneous GNN with its skip connection
(0.12\%). The MLP baseline fails without any spatial structure ($r = 0.35$),
confirming that the lattice topology is essential.

\subsection{Coupling Generalization}
\label{sec:generalization}

To test whether the model has learned a transferable action functional rather
than memorizing a single action landscape, we evaluate the model trained at
$\msq = -0.5$ on configurations generated at different $\msq$ values (same
$\lambda = 0.5$).

\begin{table}[t]
\centering
\caption{Generalization across coupling values. The model was trained at
$\msq = -0.5$ (marked $\ast$) and evaluated on configurations at other
$\msq$ values without retraining.}
...
Table~\ref{tab:generalization} shows that the model generalizes well to nearby
couplings: for $|\msq - \msq_\text{train}| \leq 0.5$, the Pearson correlation
remains above 0.999, indicating the learned action functional transfers across
this range. Performance degrades gracefully for more distant couplings, with
$r = 0.99$ at $\msq = -1.0$ and $r = 0.93$ at $\msq = -1.5$. The sharp drop at
$\msq = -2.0$ ($r = 0.58$) coincides with the approach to the critical region
($\msqc \approx -2.45$), where the field distribution changes qualitatively and
extrapolation from the symmetric phase becomes unreliable. Training on a mixture
of couplings would likely extend the generalization range substantially.
...
differences. However, near the critical point where $\xi \gg \ell$, the model
cannot capture the long-range correlations that dominate critical behavior.
The observed performance degradation at criticality
(Sec.~\ref{sec:generalization}) is therefore not merely a limitation but
\emph{expected physical behavior}: it confirms that the network has learned a
genuinely local operator rather than memorizing global patterns. Increasing $B$
enlarges the locality scale but risks over-smoothing, where distinct node
representations collapse toward a common value. For theories requiring larger
locality scales, possible mitigations include multi-scale pooling layers, skip
connections across non-adjacent blocks, or attention mechanisms that allow
selective long-range communication.
...
gauge-equivariant architectures~\cite{Favoni2022,Luo2021} achieve exact
symmetry by constraining the network weights, but this rigidity restricts them
to regular lattices with specific boundary conditions. Our approach prioritizes
\emph{architectural flexibility}---the same model structure handles irregular
meshes, non-periodic boundaries, and adaptive lattice refinements without
...
Quantifying the learned equivariance error on regular lattices is left to
future work.
